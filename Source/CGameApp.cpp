...
bool CGameApp::BuildObjects()
{
	...
	m_pPlayer2 = new CPlayer(m_pBBuffer, "data/ship2rr.bmp");
	...
	m_blueWins = new Sprite("data/bluewins.bmp", RGB(0xff, 0x00, 0xff));
	m_redWins = new Sprite("data/redwins.bmp", RGB(0xff, 0x00, 0xff));
...
//	addEnemies(ceil((m_screenSize.x - 600) / 100) * 4);
	...
	m_blueWins->setBackBuffer(m_pBBuffer);
	m_redWins->setBackBuffer(m_pBBuffer);
...
void CGameApp::SetupGameState()
{
	...
	m_blueWins->mPosition = Vec2(int(m_screenSize.x / 2), int(m_screenSize.y / 2));
	m_redWins->mPosition = Vec2(int(m_screenSize.x / 2), int(m_screenSize.y / 2));
...
void CGameApp::ReleaseObjects( )
{
	...
	while (!m_livesBlue.empty()) delete m_livesBlue.front(), m_livesBlue.pop_front();
...
void CGameApp::ProcessInput( )
{
	...
	{
		if (pKeyBuffer[VK_UP] & 0xF0) Direction1 |= CPlayer::DIR_FORWARD;
		if (pKeyBuffer[VK_DOWN] & 0xF0) Direction1 |= CPlayer::DIR_BACKWARD;
		if (pKeyBuffer[VK_LEFT] & 0xF0) Direction1 |= CPlayer::DIR_LEFT;
		if (pKeyBuffer[VK_RIGHT] & 0xF0) Direction1 |= CPlayer::DIR_RIGHT;
		if (pKeyBuffer[VK_SPACE] & 0xF0 && m_pPlayer2->frameCounter() >= 50)
		{
			fireBullet(m_pPlayer2->Position(), Vec2(0, 250), 3);
	...
	if (!m_pPlayer->isDead)
	{
		...
		if (pKeyBuffer['Z'] & 0xF0 && m_pPlayer->frameCounter() >= 50)
		{
...
void CGameApp::DrawObjects()
{
	...
	//	m_scoreP2->draw();
		...
		for (auto lg : m_livesBlue) lg->draw();
...
bool CGameApp::detectBulletCollision(const Sprite* bullet)
{
	...
	/*if (bulletCollision(*bullet, *m_pPlayer) && bullet->team == 3 && !m_pPlayer->hasExploded() && m_livesBlue.size() > 0)
	{
		m_pPlayer->takeDamage();
		
		delete m_livesBlue.back();
		m_livesBlue.pop_back();
	...
	if (bulletCollision(*bullet, *m_pPlayer2) && bullet->team == 2 && !m_pPlayer2->hasExploded() && m_livesBlue.size() > 0)
	{
		m_pPlayer2->takeDamage();
	...
	if (bulletCollision(*bullet, *m_pPlayer) && bullet->team == 3 && !m_pPlayer->hasExploded() && m_livesRed.size() > 0)
	{
		m_pPlayer->takeDamage();
...
void CGameApp::fireBullet(const Vec2 position, const Vec2 velocity, int x)
{
	if (x == 1)
	{
		bullets.push_back(new Sprite("data/bullet.bmp","data/bulletmask.bmp" ));
	}
	else if (x == 2)
	{
		bullets.push_back(new Sprite("data/bullet.bmp","data/bulletmask.bmp" ));
	...
	{
		bullets.push_back(new Sprite("data/bulletaliens.bmp","data/bulletaliensmask.bmp"));
	} 
...
void CGameApp::setPLives(int livesP1, int livesP2)
{
	...
	for (int it = 0; it != livesP1; ++it) {
		m_livesBlue.push_back(new Sprite("data/heart.bmp", RGB(0xff, 0x00, 0xff)));
		auto& lastGreen = m_livesBlue.back();
...
{/*
	...
	if (m_pPlayer->isDead && m_gameState && m_gameState == ONGOING)
	{
		m_gameState = REDWINS;
	}
	else 	if (m_pPlayer2->isDead && m_gameState && m_gameState == ONGOING)
	{
		m_gameState = BLUEWINS;
	}
